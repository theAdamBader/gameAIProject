...
namespace Complete
{
    public class TankShooting : MonoBehaviour
    {
		...
		public float fireRate = 0.8f;				// How slow or fast the shots are
		private float nextShot = 0.0f;				// How long each shot shall take
        ...
        private void Update ()
        {
            ...
            switch (m_FiringState) {
                ...
                case FIRING_INACTIVE:
					...
					if (m_AIFireRequest && Time.time > nextShot) {
					...
					if (Input.GetButtonDown (m_FireButton) && Time.time > nextShot) {
						nextShot = Time.time + fireRate;
                            m_FiringState = FIRING_START;
                        }
                    }
